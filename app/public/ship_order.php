...
if ($orderId <= 0) {
    header("Location: {$redirectBase}?msg=shipped_fail");
    exit;
}
...
try {
    ...
    if ($order['status'] !== 'reserved') {
        throw new Exception("Sadece 'reserved' siparişler kargoya verilebilir.");
    }
    ...
    // 3) Yeterli reserved stok var mı?
    ...
    // 5) Sipariş durumunu 'shipped' yap
    $updateOrderSql = "
        UPDATE orders
        SET status = 'shipped'
        WHERE id = :id
    ...
    header("Location: {$redirectBase}?msg=shipped_success");
    exit;
} catch (Exception $e) {
