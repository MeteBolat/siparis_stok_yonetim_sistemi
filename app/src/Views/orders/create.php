...
function createRow(index) {
  ...
  btnDel.onclick = () => { tr.remove(); recalcTotals(); };
...
function recalcTotals() {
  ...
  rows.forEach(row => {
    ...
    if (select.value) {
      unitPrice = parseFloat(select.selectedOptions[0].dataset.price || '0');
    }
...
document.getElementById('add-row').addEventListener('click', () => createRow(rowIndex++));
createRow(rowIndex++);
</script>
